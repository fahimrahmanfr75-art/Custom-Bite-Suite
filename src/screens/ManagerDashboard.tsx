...
import { AppButton, Card3D, Field, OrderStatusBadge, Pill, ScreenCard, SectionTitle, commonStyles } from '../components/common';
...
function calculateDishPriceWithMandatory(dish: Dish | null | undefined) {
  if (!dish) {
    return 0;
  }

  const mandatoryExtraCost = (dish.ingredients || [])
    .filter((ingredient) => ingredient.isMandatory && ingredient.extraPrice > 0)
    .reduce((sum, ingredient) => sum + ingredient.extraPrice, 0);

  return dish.price + mandatoryExtraCost;
}
    ...
    <View style={styles.page}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.tabRow}>
          {(['profile', 'command', 'menu', 'finance', 'history'] as ManagerTab[]).map((tab) => (
                  ...
                  pendingOrders.map((order) => (
                    <Card3D key={order.id} style={styles.panel}>
                      ...
                      <View style={styles.actionRow}>
                        <AppButton label="Accept" onPress={() => void moveOrderToNextStatus(order.id)} />
                        <AppButton
                          label="Reject"
                          variant="danger"
            ...
            {editingDish ? (
              <View style={styles.stack}>
                <SectionTitle
                  ...
                  action={<AppButton label="Back to menu" variant="ghost" onPress={() => setEditingDish(null)} />}
                />
                ...
                <Card3D style={styles.dishDetailsCard}>
                  ...
                  <View style={styles.dishContent}>
                    <View style={styles.dishHead}>
                      ...
                      <Text style={styles.dishPrice}>${Number(editingDish.price || 0).toFixed(2)}</Text>
                    </View>
                ...
                <Card3D style={styles.panel}>
                  ...
                  />
                  <AppButton label="Upload image from device" variant="secondary" onPress={() => void pickDishImage()} />
                  <Field
                    label="Dish name"
                  ...
                  <Field
                    label="Base price"
                    value={String(editingDish.price)}
                    onChangeText={(value) => updateEditingDish((current) => ({ ...current, price: Number(value) || 0 }))}
                ...
                <Card3D style={styles.panel}>
                  <SectionTitle
                    ...
                    action={<AppButton label="Add category" variant="secondary" onPress={addIngredientCategory} />}
                  />
                  ...
                  <View style={styles.actionRow}>
                    ...
                    <AppButton label="Save dish" onPress={() => void saveDish()} />
                  </View>
                </Card3D>
              </View>
            ) : (
                    ...
                    <Card3D key={dish.id} style={styles.menuCard}>
                      ...
                      <View style={styles.dishContent}>
                        <View style={styles.dishHead}>
                          ...
                          <Text style={styles.dishPrice}>${calculateDishPriceWithMandatory(dish).toFixed(2)}</Text>
                        </View>
                        ...
                        <View style={styles.actionRow}>
                          <AppButton label="Edit" variant="secondary" onPress={() => openDishEditor(dish)} />
                          <AppButton label="Delete" variant="danger" onPress={() => void removeMenuDish(dish.id)} />
                        </View>
                      </View>
                    </Card3D>
                  ))}
                ...
                banners.map((banner) => (
                  <Card3D key={banner.id} style={styles.panel}>
                    ...
                    <View style={styles.actionRow}>
                      ...
                      />
                      <AppButton label="Remove" variant="danger" onPress={() => void removeBanner(banner.id)} />
                    </View>
                  </Card3D>
                ))
              )}
          ...
          <ScreenCard style={styles.modalCard}>
            ...
            {editingBanner ? (
              <ScrollView contentContainerStyle={styles.modalFormContent}>
                ...
                />
                <View style={styles.actionRow}>
                  <AppButton label="Cancel" variant="ghost" onPress={() => setEditingBanner(null)} />
                  <AppButton
                    label="Save Banner"
